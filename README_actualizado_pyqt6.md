...
- `qt6-translations-l10n`: Proporciona traducciones para diálogos de Qt, como abrir archivos, botones y ventanas internas, según el idioma del sistema.
...
```bash
python3 html_blogger_fixer.py
```

## 4. Ejecutar el programa en Linux

En distribuciones Linux Debian o basadas, o Ubuntu y basadas, busca la carpeta donde está el archivo `html_blogger_fixer.py` y luego abre una **terminal en esa carpeta** y ejecuta:
...
# 1) Uso de `tag_markdown_gui_pyqt6.py` (versión gráfica en PyQt6)

Esta versión es ideal si prefieres trabajar de forma visual. La GUI fue portada de Tkinter a **PyQt6** para integrarse mejor con Linux y Windows.
...
```bash
python3 tag_markdown_gui_pyqt6.py
```

2. Se abrirá una ventana con:

   * Un botón **"Buscar archivo .md"**
   * Un selector de lenguaje
   * Un botón **"Procesar"**
   * Un botón **"Acerca de..."**

3. Haz clic en **"Buscar archivo .md"** y selecciona tu archivo Markdown.

4. Elige el lenguaje que deseas aplicar a los bloques de código sin etiqueta:
...
Este nuevo archivo ya tiene las cajas de código sin lenguaje correctamente etiquetadas y queda listo para convertirse con Pandoc.

## Detalle importante

La versión PyQt6 **solo etiqueta bloques que no tienen lenguaje**.  
...
Pero no modifica bloques que ya tienen etiqueta, por ejemplo:
...
En Debian, MX Linux, Ubuntu y derivados se recomienda instalar:

```bash
sudo apt install qt6-translations-l10n
```

Esto ayuda a que ventanas internas de Qt, como el diálogo para abrir archivos, aparezcan en el idioma configurado en el sistema.

## Tamaño del diálogo de selección de archivo

La ventana de selección de archivo se abre más grande usando:

```python
dialogo.resize(900, 600)
```

Si quieres hacerla más grande, cambia esa línea por ejemplo a:
...
La ventana principal se centra en pantalla con el método:
...
(o usando `tag_markdown_gui_pyqt6.py`)
